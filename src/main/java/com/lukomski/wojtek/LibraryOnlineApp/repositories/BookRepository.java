...
public interface BookRepository extends JpaRepository <Book,Integer> {
    ...
    void updateDateOfRentWithAvailability(boolean available, LocalDate dateOfStartRent, LocalDate dateOfEndRent, Integer bookId);

    @Modifying
    @Transactional
    @Query(value = "UPDATE Book book SET available=?1 WHERE id=?2")
    void updateAvailability(boolean available, Integer bookId);
